...
async function generateHTML(env, kvItems) {
  const itemsWithCoupon = await Promise.all(
    kvItems.map(async (item) => {
      ...
      const couponValue = match ? Number(match[1]) : -1;
...
// ------------------- Worker 主体 -------------------
export default {
  // 用户访问网页时直接从 TIME KV 读取
  async fetch(request, env) {
    try {
      const htmlCache = await env.TIME.get("htmlCache");
      if (!htmlCache) {
        return new Response("暂无缓存，请稍后", { status: 500 });
      }
  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      ...
      const html = await generateHTML(env, kvItems);
      await env.TIME.put("htmlCache", html);

      console.log("✅ 已完成十分钟更新任务，共处理", kvItems.length, "条");
    } catch (err) {
